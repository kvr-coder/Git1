...
set "B64=C:\Windows\Temp\git1install.b64"
set "PS1=C:\Windows\Temp\git1install.ps1"
...
certutil -decode %B64% %PS1% >/dev/null 2>/dev/null
if exist "%B64%" del "%B64%"

if not exist "%PS1%" (
  echo.
  echo ERROR: Failed to extract installer. Please contact support.
  pause
  exit /b 1
)
